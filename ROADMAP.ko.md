...
재구성한 단계입니다. 날짜는 각 단계를 대표하는 커밋이 들어간 시점이고, 여러 단계가 깔끔하게
순차적으로 끝나지 않고 서로 겹칩니다(특히 Socket.IO→GraphQL 마이그레이션은 완전히 정착되기까지
약 5개월이 걸렸습니다). 커밋 단위 전체 기록은 [CHANGELOG.md](CHANGELOG.md)를 참고하세요.
...
2. **Socket.IO → GraphQL 메시징 마이그레이션** (2026-01-22 ~ 2026-06-11) — 가장 오래 걸린
   단계이며, 한 번의 깔끔한 전환이 아니었습니다. GraphQL 메시지 전달 테스트는 2026-01-22에
   시작했고("Testing Socket through GraphQL real time responses"), `sendMessage`의 초기 트랜잭션
   구현은 2026-03-01에 들어갔으며, 기존 Socket.IO 메시지 핸들러는 2026-06-11에야 실제로
   삭제되었습니다("unused WS sendMessage handler and Socket.IO broadcast removed"). 즉 두 경로가
   약 4.5개월간 공존하다가 그제서야 GraphQL이 유일한 메시지 전달 경로가 된 것입니다.
...
3. **배포 인프라** (2026-04-22 ~ 2026-05-27) — CI/CD 워크플로우는 2026-04-22에, Docker Compose는
   2026-05-01에 추가되었고, 이어진 모노레포 재구성(2026-05-13 ~ 2026-05-27)을 거쳐 지금의
...
README의 옛 "향후 확장 계획" 절에서 옮겨온 백로그입니다. 확정된 타임라인이나 우선순위가
아닙니다.
