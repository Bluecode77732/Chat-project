...
- **지속성 검증은 실제 배포 후에만 가능한 테스트이며, 아직 수행하지 않았습니다.** `pnpm lint`/
  `pnpm test`와 로컬 `pnpm start:dev` 스모크 테스트는 코드 로직(폴백 경로, 회귀 없음)만 확인할 뿐,
  이 ADR이 실제로 주장하는 바 -- 재배포해도 로그가 살아남는다는 것 -- 은 확인하지 못합니다. 이는
  Railway의 실제 볼륨 마운트·컨테이너 재시작 동작에 달려 있고, 어떤 로컬 환경으로도 재현할 수
  없기 때문입니다. 볼륨이 연결된 이후에만 가능한 남은 단계: 5xx를 하나 발생시켜 `error.logs.log`에
  기록되는지 확인 → 재배포 → 그 항목이 여전히 남아있는지 확인.