...
- 조용한 토큰 갱신은 전부 `refreshAccessTokenSafely()`
  ...
  진행 중인 요청 하나를 공유합니다. 그 덕분에 두 번째 호출자가 리다이렉트 도중 충돌하는 계정을
  잘못 채택하는 레이스 컨디션이 생기지 않습니다.
- 탭 사이의 신원 충돌은 별도로 방어합니다. `sessionStorage['chat:sessionUserId']`에 이 탭이
  마지막으로 인증한 계정을 기록해 둡니다. `refreshToken` 쿠키는 탭끼리 공유되므로, 갱신 결과가
  다른 계정으로 나오면 다른 탭에서 다른 계정으로 로그인했다는 뜻입니다. 이때는 새 신원을
...
- 고려했다가 배제한 대안:
  ...
  - **리프레시 없는 단일 장기 토큰**: 만료를 길게 잡으면 토큰이 탈취됐을 때 수명 내내 유효해서
    블랙리스트 말고는 무효화할 방법이 없습니다. 짧게 잡으면 조용한 갱신이 없는 한 사용자가
    수시로 재로그인해야 합니다. 어느 쪽이든 나쁜 선택지입니다. 액세스/리프레시를 분리하면
    액세스 토큰을 짧게 유지하면서도 이 UX 비용이 사라집니다.
