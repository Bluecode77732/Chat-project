...
- 탭 간 신원은 별도로 보호됩니다. `sessionStorage['chat:sessionUserId']`가 이 탭이 마지막으로
  인증했던 계정을 기록하는데, `refreshToken` 쿠키는 탭 간에 공유되므로 갱신 결과가 다른 계정으로
  풀린다면 형제 탭이 다른 계정으로 로그인했다는 뜻입니다 — 이 경우 새 신원을 조용히 승계하는 대신
  해당 탭을 로그아웃시킵니다.
- 고려했다가 배제한 대안:
