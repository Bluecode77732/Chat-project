...
첫째, `envFilePath` 선택(`app.module.ts:72-77`, 어떤 `.env*` 파일을 읽을지)입니다.
둘째, HTTP 호스트 바인딩(`main.ts:103-111`, `127.0.0.1` vs `0.0.0.0`)입니다.
...
전용 파일"이라는 의미로 재사용되고 있었습니다. 이는 `envFilePath`의 3분기 로직과 충돌하고,
표준 관례를 아는 사람에게는 혼동을 줄 수 있었습니다.
...
`'docker'`를 직접 비교하는 게 아니라 `=== 'development'`에 대한 암묵적 else였기 때문입니다.
...
- `docker-compose.yml`의 `chat` 서비스는 `NODE_ENV: development` + `RUNTIME_ENV: docker`를
  ...
  에러노출 면에서 맨몸 로컬 개발과 동일하게 동작하도록 하기 위해서입니다. 이렇게 하면 위에서
  설명한 Sentry 오분류 문제를 피할 수 있습니다. 이제 파일 선택과 호스트 바인딩 차이는
  `RUNTIME_ENV=docker` 하나만이 결정합니다.
...
- 고려했다가 배제한 대안:
  - **`NODE_ENV=docker`를 유지하고 `.env.local` → `.env.docker`로 이름만 변경**: 배제했습니다.
    ...
    네 번째 값을 특별 취급해야 합니다. 배경에서 설명한 우연한 호스트 바인딩 안전성도
    문서화되지 않은 채로 남아, `main.ts`가 나중에 수정되면 깨지기 쉬운 상태입니다.
