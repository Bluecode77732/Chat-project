...
오리진에서 실행됨. 둘 다 httpOnly `refreshToken` 쿠키를 크로스 오리진으로 보내야 하므로
`credentials: true`와 정확한 오리진 허용 목록이 필요함. `origin: '*'`는 자격 증명이
포함된 요청과 애초에 호환되지 않으며, 이 프로젝트에서는 어떤 상황에서도 Never Do임.
...
- 두 프론트엔드 모두 httpOnly `refreshToken` 쿠키(`withCredentials` / `credentials: 'include'`)에
  의존하므로 `credentials: true`가 함께 필요함.
- 고려했다가 배제한 대안:
  - **앱별로 CORS 설정을 두 개 두고 요청 시점에 분기**: 배제함. 어느 프론트엔드가
    호출했는지 백엔드가 먼저 식별해야 올바른 정책을 고를 수 있는데, 콤마 구분 허용 목록
    하나면 정적으로 끝나는 문제임.
  - **`origin: '*'` + `credentials: false`**(쿠키 기반 인증을 포기하고 와일드카드 허용):
    배제함. `frontend`와 `admin`이 둘 다 의존하는 httpOnly `refreshToken` 흐름이 깨지고,
    인증 방식 자체를 통째로 바꿔야 함.
  - **와일드카드/정규식 오리진 매칭**(예: `*.vercel.app` 서브도메인 전부 허용): 배제함.
    이 프로젝트의 알려진 배포 두 개만이 아니라 Vercel에 호스팅된 아무 앱이나 요청을 보낼 수
    있게 되어 지나치게 허용적임.

## 결과

- `origin: '*'`는 절대 제안하지 않음. `credentials: true`와 호환되지 않고, 허용 목록의 목적
  자체를 무력화함.
- `CORS_ORIGIN`을 읽는 대신 `main.ts`에 오리진을 하드코딩하는 것도 절대 제안하지 않음.
  허용 목록의 기준이 두 곳으로 갈라지기 때문임.
- 세 번째 프론트엔드/admin 소비자(예: 향후 모바일 웹 클라이언트)를 추가할 때는 모든 배포 환경의
  `CORS_ORIGIN`에 해당 오리진을 반드시 추가해야 함. 빠뜨리면 동작하지 않으며 폴백은 없음.
