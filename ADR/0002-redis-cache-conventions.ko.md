...
- 고려했다가 배제한 대안:
  - **`ioredis` 옆에 두 번째 클라이언트(`node-redis`)를 두는 것**: 의도적으로 고려한 게 아니라
    실제로 실수로 벌어졌던 대안입니다 — 플래그된, 쓰이지 않는 `redis` v5 의존성은
    [ARCHITECTURE.md](../ARCHITECTURE.md#resolved-anomaly) 참고. 이 컨벤션이 막으려는 바로 그
    혼란("어느 클라이언트가 정본인가?")의 구체적 사례로 여기 남겨둡니다.
  - **네이밍 컨벤션 없는 자유 형식 키**: 배제 — `{service}:{entity}:{id}` 형태가 공유되지 않으면
    키 충돌을 감지하거나 어느 키가 어떤 TTL을 갖는지 감사하려면 모든 호출부를 하나하나 읽어야
    합니다.
  - **TTL 없이 별도 스케줄 정리 작업으로 대체**: 배제 — 운영해야 할 프로세스가 하나 늘고, 그
    작업이 도는 사이에는 여전히 무한정 증가할 창이 남습니다. 쓰기 시점에 원자적으로 TTL을 거는
    쪽은 별도 프로세스도, 그런 창도 필요 없습니다.
