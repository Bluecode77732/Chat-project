...
이 저장소에 변경사항을 기여하기 위한 진입점임. 설치, 브랜치 모델, 커밋 컨벤션, PR
체크리스트를 다룸. README의 빠른 시작은 앱을 로컬에서 실행하는 방법까지만 다루고, 변경사항을
제출하는 워크플로우는 다루지 않음. 코드 수준의 깊은 컨벤션(Never Do 규칙, 테스트 패턴,
트랜잭션 경계)은 CLAUDE.md에 이미 상세히 문서화되어 있음. 이 문서는 그 내용을 다시 쓰지
않음. 새 기여자가 PR을 열 수 있는 지점까지 데려다주는 진입로 역할만 하고, 그 이후는
CLAUDE.md로 안내함.
...
시점에 Joi로 검증되므로(`app.module.ts`), 하나라도 빠지면 조용히 넘어가지 않고 바로 실패함.
...
- `main` — 배포 브랜치임. `.github/workflows/deploy.yml`의 `deploy` job은 `main` push에서만
  트리거되어 Railway로 배포함.
- `dev` — 활성 개발 브랜치임.

(리모트에 보일 수 있는 `dev1`이나 자동 생성되는 `railway/code-change-*` 같은 다른 브랜치는 여기
문서화된 워크플로우에 포함되지 않음. 그런 브랜치에서 분기하려면 먼저 메인테이너에게
확인하세요.)

## 커밋 컨벤션

최근 히스토리는 `Prefix: description` 스타일을 씀. 대문자로 시작하는 접두어, 콜론, 짧은 설명
순서임. 앞으로는 다음 중 하나를 사용하세요:
...
전혀 돌지 않음**. PR 제목은 커밋과 동일한 `Prefix: description` 컨벤션을 사용하세요. PR
템플릿이나 필수 리뷰어 규칙은 없으며, 아래 CI 잡들이 그 관문 역할을 함.

`.github/workflows/deploy.yml`은 `main`으로의 모든 PR에서 실행됨.
...
`test` 잡의 마지막 네 단계는 문서 무결성 검사이며, 테스트와 똑같이 실패하면 빌드를 막음:
...
`check:changelog`는 기록되지 않은 최신 커밋을 **하나만** 허용함. 기록을 수행하는 그 커밋
자신임. 커밋이 하나뿐이면 CHANGELOG를 건드릴 필요가 없지만, 그 위에 두 번째 커밋을 쌓는
순간 첫 번째 커밋에서 검사가 실패함. PR에 커밋이 둘 이상이라면 각 커밋의 제목 줄을 그 커밋
...
> (`docker-compose.yml`)와 문서상의 로컬 사전 요구사항은 `postgres:18`을 사용함. 드리프트가
> 아니라 의도된 환경 차이임.

PR을 올리기 전에 로컬에서, backend 쪽만이 아니라 필수 통과 잡인 `test`가 실행하는 전부를
돌리세요. 그러지 않으면 나머지 여섯 단계가 CI에서 먼저 깨짐:
```bash
cd backend
...
트랜잭션 경계 등)을 [CLAUDE.md](CLAUDE.md#never-do--forbidden-patterns)에 문서화해 둠.
...
- 테스트는 소스 파일 옆에 `*.spec.ts`로 둠. 커버리지는 서비스와 Redis 모듈만 측정함
  (`backend/package.json`의 `coveragePathIgnorePatterns`가 controller, guard, gateway, resolver,
  interceptor, DTO, entity 등을 제외함 — 의도된 정책이지 빈틈이 아님).
- `bcrypt`는 `backend/src/mocks/bcrypt.ts`를 통해 전역으로 mock 처리됨.
- 실제 DB를 두드리지 말고 리포지토리를 mock하세요. 패턴은
  [CLAUDE.md의 Testing 절](CLAUDE.md#testing)에 있음.
...
- `admin/`(vitest, 14개)과 `frontend/`(vitest, 21개) 둘 다 이제 유닛테스트를 갖추고 있으며,
  동일한 설정(`src/test/setup.ts`, 동일 버전의 devDependency)을 씀. `frontend/` 스위트는
  admin의 기존 3개 파일(axios/protected-route/auth.store — frontend의 더 단순한, role 없는
  인증 모델에 맞게 조정)을 포팅하고, `session-guard.ts`용 테스트를 새로 추가함.
  in-flight 리프레시 중복 방지와 탭 간 계정 충돌 감지 로직임. 두 앱 모두 테스트가 없었지만,
  인증 코드에서 레이스 컨디션에 가장 민감한 부분임(CLAUDE.md의 Session Guard 절 참고).
- `backend/test/app.e2e-spec.ts`는 `Test.createTestingModule({ imports: [AppModule] })` +
  ...
  거치지 않음. 그래서 `cookieParser()`, `helmet()`, 전역 `ValidationPipe`,
  `AllExceptionsFilter`가 이 테스트들에는 적용되지 않음. 고치지 않고 그대로 둔 이유: 이
  파일의 네 케이스는 전부 이런 것들에 의존하지 않도록(라우팅 + 가드/서비스 단의
  `HttpException`만 쓰도록) 일부러 골랐기 때문임. e2e에서 `main.ts`의 middleware 스택까지
...
`.github/ISSUE_TEMPLATE/architecture-completeness-sweep.md` 하나뿐임. 범용 버그/기능
