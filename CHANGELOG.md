...
All notable history of this project, reconstructed from `git log` in full — every commit, grouped by
date (newest first). This project does not use semantic versioning yet (no git tags exist as of this
writing), so entries are grouped by date rather than by release version.

Commit messages are reproduced exactly as written, including inconsistent casing and informal entries
from early development (see [CONTRIBUTING.md](CONTRIBUTING.md#commit-convention) for the convention new
